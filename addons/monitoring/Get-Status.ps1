...
kubectl wait --timeout=5s --for=condition=Available -n monitoring deployment/kube-prometheus-stack-kube-state-metrics 2>&1 | Out-Null
...
kubectl wait --timeout=5s --for=condition=Available -n monitoring deployment/kube-prometheus-stack-operator 2>&1 | Out-Null
...
kubectl wait --timeout=5s --for=condition=Available -n monitoring deployment/kube-prometheus-stack-plutono 2>&1 | Out-Null
...
kubectl rollout status statefulsets -n monitoring --timeout=5s 2>&1 | Out-Null
...
kubectl rollout status daemonsets -n monitoring --timeout=5s 2>&1 | Out-Null
