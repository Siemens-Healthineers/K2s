...
&$global:KubectlExe wait --timeout=5s --for=condition=Available -n kubernetes-dashboard deployment/dashboard-metrics-scraper 2>&1 | Out-Null
...
&$global:KubectlExe wait --timeout=5s --for=condition=Available -n kubernetes-dashboard deployment/kubernetes-dashboard 2>&1 | Out-Null
