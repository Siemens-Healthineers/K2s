...
function Test-PluginImageLayout {
    <#
    .SYNOPSIS
    Validates the built image filesystem contains /plugins/<pluginDir>/main.js,
    ...
    $expected = "plugins/$PluginDir/main.js"
    $contents = & $CraneExe export $OciTar - | & tar -tzf - 2>$null
    if (-not $contents) { $contents = & $CraneExe export $OciTar - | & tar -tf - 2>$null }
    if (($contents | Where-Object { $_ -match [regex]::Escape($expected) }).Count -eq 0) {
        throw "[HlPlugin] Built image '$OciTar' does not contain '/$expected' (runtime contract violated)"
    }
    Write-HlPluginLog "Image layout OK: /$expected present"
}
