...
**Default SMB volume layout.** The `ceph-smb` StorageClass keeps the SMB CSI driver's default
volume folder handling instead of forcing a custom `<namespace>/<name>` path. This preserves the
standard per-volume layout generated by the driver while still exposing the shared CephFS subvolume
through SMB. The subvolume is created with `--extra-config "force create mode = 0777, force
directory mode = 0777"` so files created by one OS remain fully readable/writable from the other.
...
3. creates the `smbcreds` Secret (the generated Samba user/password) and generates the `ceph-smb`
   ...
   with source `//<clusterHostIp>/<shareName>` and the default SMB CSI volume folder layout.
...
The `ceph-smb` StorageClass keeps the default SMB CSI volume folder layout, so each PVC uses the
standard folder structure the driver creates under the shared subvolume. This avoids custom
namespace/name path injection while still exposing the same shared CephFS content over SMB. The
subvolume is exported with `force create mode = 0777, force directory mode = 0777` so files created
by one OS stay accessible from the other.
