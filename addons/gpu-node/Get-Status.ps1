...
kubectl wait --timeout=5s --for=condition=Available -n gpu-node deployment/nvidia-device-plugin 2>&1 | Out-Null
...
kubectl rollout status daemonset dcgm-exporter -n gpu-node --timeout=5s 2>&1 | Out-Null
