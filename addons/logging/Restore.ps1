...
# Only restore genuinely stateful config (OpenSearch). The Fluent Bit ConfigMaps
# (fluent-bit, fluent-bit-win-parsers, fluent-bit-win-config) are intentionally NOT restored:
# they are regenerated by Enable.ps1 (run earlier in the restore flow) with the correct,
# restore-host log-root substitution. Re-applying a backed-up snapshot would reintroduce stale,
# host-specific paths (e.g. D:\Temp\k2s-logs captured on a different host). Older backups may still
# contain fluent-bit*.json files; those are simply ignored because they are not listed here.
$filesToApply = @(
