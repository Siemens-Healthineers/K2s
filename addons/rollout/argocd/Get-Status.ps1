...
$statusProps = @($isArgoCDApplicationsetControllerRunningProp,$isArgoCDDexServerRunningProp ,$IsArgoCDNotificationControllerRunningProp, $isArgoCDRedisDBRunningProp, $isArgoCDRepoServerRunningProp, $isArgoCDServerRunningProp ,$areStatefulsetsRunningProp)

# Append GitOps addon controller status if deployed
if (Test-AddonGitOpsControllerDeployed) {
    $controllerStatus = Get-AddonGitOpsControllerStatus -ImplementationName 'argocd'
