...
<#
.SYNOPSIS
Uninstalls ingress nginx-gateway from the cluster

.DESCRIPTION
Ingress nginx-gateway is using k8s load balancer and is bound to the IP of the master machine.
...
if ($null -eq (Invoke-Kubectl -Params 'get', 'namespace', 'nginx-gateway', '--ignore-not-found').Output -and (Test-IsAddonEnabled -Addon ([pscustomobject] @{Name = 'ingress'; Implementation = 'nginx-gateway' })) -ne $true) {
...
Remove-AddonFromSetupJson -Addon ([pscustomobject] @{Name = 'ingress'; Implementation = 'nginx-gateway' })

# adapt other addons
Update-Addons -AddonName $addonName

Write-Log 'Uninstallation of ingress nginx-gateway-fabric addon finished' -Console
