...
if ($isIngressNginxGatewayRunningProp.Value -eq $true) {
    ...
    $isIngressNginxGatewayRunningProp.Message = "The nginx ingress gateway api controller is not working. Try restarting the cluster with 'k2s start' or disable and re-enable the addon with 'k2s addons disable ingress nginx-gateway' and 'k2s addons enable ingress nginx-gateway'"
} 
...
if ($isExternalIPSetProp.Value -eq $true) {
    $isExternalIPSetProp.Message = "The external IP for nginx-gateway service is set to $controlPlaneIp"
}
else {
    $isExternalIPSetProp.Message = "The external IP for nginx-gateway service is not set properly. Try restarting the cluster with 'k2s start' or disable and re-enable the addon with 'k2s addons disable ingress nginx-gateway' and 'k2s addons enable ingress nginx-gateway'"
}
