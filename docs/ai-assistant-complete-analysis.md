...
| Cost per query | 💰 $0.001-$0.06/query | 💰 $0 (electricity only) |
...
1. Loads the new model while the old one serves requests
2. Atomic cutover once the new model is ready
3. No downtime
...
1. Pre-packaged as container images in the K2s offline package
2. Exposed as in-cluster services (not external processes)
3. Accessible via the K8s API server proxy (same pattern as HolmesGPT)
...
| **L3 — Autonomous responder** | Acts on alerts without human input (low-risk changes) | 🟡 Future (12-18 months) |
