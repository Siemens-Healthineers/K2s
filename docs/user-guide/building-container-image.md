...
## `k2s build` Internals
Under the hood of `k2s build` is the *PowerShell* script [Build-Image.ps1](https://github.com/Siemens-Healthineers/K2s/blob/main/lib/scripts/windows/host/image/Build-Image.ps1){target="_blank"} (and its batch-wrapper, both part of the *K2s* setup) to automate the container build. A container image will be created in your local repository.
