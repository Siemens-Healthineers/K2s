...
When you enable `rollout fluxcd`, *K2s* installs the host Flux CLI at `bin\\flux.exe`. Disabling `rollout fluxcd` removes it. In offline environments the CLI zip is bundled in the addon export and extracted on import.
