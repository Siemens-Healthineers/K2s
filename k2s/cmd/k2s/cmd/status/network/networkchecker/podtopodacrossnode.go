...
type PodToPodAcrossNodeHandler struct {
	BaseNetworkChecker
...
func NewPodToPodAcrossNodeHandler(client clusterclient.K8sClientSet, pod1, pod2 clusterclient.PodSpec) *PodToPodAcrossNodeHandler {
	return &PodToPodAcrossNodeHandler{client: client, pod1: pod1, pod2: pod2}
}

func (p *PodToPodAcrossNodeHandler) CheckConnectivity() (*NetworkCheckResult, error) {

	slog.Debug("Checking connectivity between pod %s and pod %s across nodes\n", p.pod1.PodName, p.pod2.PodName)
