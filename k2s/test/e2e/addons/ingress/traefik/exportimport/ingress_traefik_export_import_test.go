...
var _ = BeforeSuite(func(ctx context.Context) {
	...
	GinkgoWriter.Println("INGRESS TRAEFIK EXPORT/IMPORT TEST - SETUP")
	GinkgoWriter.Println("========================================")
...
var _ = Describe("ingress traefik addon export and import", Ordered, func() {
	Describe("export ingress traefik addon", func() {
		...
		It("exports ingress traefik addon to versioned OCI tar file", func(ctx context.Context) {
			GinkgoWriter.Println(">>> TEST: exports ingress traefik addon to versioned OCI tar file")
			exportedOciFile = exportimport.ExportAddon(ctx, suite, "ingress", "traefik", exportPath)
	...
	Describe("import ingress traefik addon", func() {
		...
		It("can be enabled when only addons/common and addons/ingress are present", func(ctx context.Context) {
			...
			DeferCleanup(func() {
				_, _ = suite.K2sCli().Exec(context.Background(), "addons", "disable", "ingress", "traefik", "-o")
			})
	...
	Describe("export and import with relative paths", func() {
		...
		BeforeAll(func(ctx context.Context) {
			absRelExportDir = filepath.Join(suite.RootDir(), "tmp", "ingress-traefik-relpath-test")
		...
		It("exports addon using a relative directory path", func(ctx context.Context) {
			relOciFile = exportimport.ExportAddonRelativePath(ctx, suite, "ingress", "traefik", suite.RootDir(), relExportDir)
