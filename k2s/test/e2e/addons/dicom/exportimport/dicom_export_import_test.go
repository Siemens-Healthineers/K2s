...
var _ = BeforeSuite(func(ctx context.Context) {
	...
	GinkgoWriter.Println("DICOM EXPORT/IMPORT TEST - SETUP")
	GinkgoWriter.Println("========================================")
...
var _ = Describe("dicom addon export and import", Ordered, func() {
	Describe("export dicom addon", func() {
		...
		It("exports dicom addon to versioned OCI tar file", func(ctx context.Context) {
			GinkgoWriter.Println(">>> TEST: exports dicom addon to versioned OCI tar file")
			exportedOciFile = exportimport.ExportAddon(ctx, suite, "dicom", "", exportPath)
		...
		It("contains dicom addon folder with correct OCI structure", func(ctx context.Context) {
			GinkgoWriter.Println(">>> TEST: contains dicom addon folder with correct OCI structure")
	...
	Describe("export and import with relative paths", func() {
		...
		BeforeAll(func(ctx context.Context) {
			absRelExportDir = filepath.Join(suite.RootDir(), "tmp", "dicom-relpath-test")
		...
		It("exports addon using a relative directory path", func(ctx context.Context) {
			relOciFile = exportimport.ExportAddonRelativePath(ctx, suite, "dicom", "dicom", suite.RootDir(), relExportDir)
