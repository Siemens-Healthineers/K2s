...
var _ = BeforeSuite(func(ctx context.Context) {
	...
	GinkgoWriter.Println("ROLLOUT FLUXCD EXPORT/IMPORT TEST - SETUP")
	GinkgoWriter.Println("========================================")
...
var _ = Describe("rollout fluxcd addon export and import", Ordered, func() {
	Describe("export rollout fluxcd addon", func() {
		...
		It("exports rollout fluxcd addon to versioned OCI tar file", func(ctx context.Context) {
			GinkgoWriter.Println(">>> TEST: exports rollout fluxcd addon to versioned OCI tar file")
			exportedOciFile = exportimport.ExportAddon(ctx, suite, "rollout", "fluxcd", exportPath)
		...
		It("contains rollout fluxcd addon folder with correct OCI structure", func(ctx context.Context) {
			GinkgoWriter.Println(">>> TEST: contains rollout fluxcd addon folder with correct OCI structure")
	...
	Describe("import rollout fluxcd addon", func() {
		...
		It("can be enabled when only addons/common and addons/rollout are present", func(ctx context.Context) {
			...
			suite.K2sCli().MustExec(ctx, "addons", "disable", "rollout", "fluxcd", "-o", "-f")
	...
	Describe("export and import with relative paths", func() {
		...
		BeforeAll(func(ctx context.Context) {
			absRelExportDir = filepath.Join(suite.RootDir(), "tmp", "rollout-fluxcd-relpath-test")
		...
		It("exports addon using a relative directory path", func(ctx context.Context) {
			relOciFile = exportimport.ExportAddonRelativePath(ctx, suite, "rollout", "fluxcd", suite.RootDir(), relExportDir)
