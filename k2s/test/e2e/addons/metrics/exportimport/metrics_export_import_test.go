...
func TestMetricsExportImport(t *testing.T) {
	RegisterFailHandler(Fail)
	RunSpecs(t, "metrics Addon Export/Import Tests", Label("addon", "addon-ilities", "acceptance", "internet-required", "setup-required", "invasive", "metrics", "export-import", "air-gapped", "system-running"))
}

var _ = BeforeSuite(func(ctx context.Context) {
	GinkgoWriter.Println("========================================")
	GinkgoWriter.Println("METRICS EXPORT/IMPORT TEST - SETUP")
	GinkgoWriter.Println("========================================")
	...
	exportPath = filepath.Join(suite.RootDir(), "tmp", "metrics-export-test")
	...
	addon = exportimport.GetAddonByName(allAddons, "metrics")
	Expect(addon).NotTo(BeNil(), "metrics addon should exist")
	GinkgoWriter.Printf("[Setup] Found addon: %s\n", addon.Metadata.Name)

	impl = exportimport.GetImplementation(addon, "metrics")
	Expect(impl).NotTo(BeNil(), "metrics implementation should exist")
...
var _ = Describe("metrics addon export and import", Ordered, func() {
	Describe("export metrics addon", func() {
		BeforeAll(func(ctx context.Context) {
			exportimport.CleanupExportedFiles(exportPath, "")
		})

		It("exports metrics addon to versioned OCI tar file", func(ctx context.Context) {
			GinkgoWriter.Println(">>> TEST: exports metrics addon to versioned OCI tar file")
			exportedOciFile = exportimport.ExportAddon(ctx, suite, "metrics", "", exportPath)
		...
		It("contains metrics addon folder with correct OCI structure", func(ctx context.Context) {
			GinkgoWriter.Println(">>> TEST: contains metrics addon folder with correct OCI structure")
			extractedArtifactsDir := exportimport.ExtractOciTar(ctx, suite, exportedOciFile, exportPath)

			expectedDirName := exportimport.GetExpectedDirName("metrics", "metrics")
	...
	Describe("clean up metrics resources", func() {
		BeforeAll(func(ctx context.Context) {
	...
	Describe("import metrics addon", func() {
		...
		It("all addon files present at correct paths after import", func(ctx context.Context) {
			...
			metricsImplDir := filepath.Join(suite.RootDir(), "addons", "metrics")
			GinkgoWriter.Printf("[Test] Metrics implementation directory: %s\n", metricsImplDir)
		...
		It("can be enabled when only addons/common and addons/metrics are present", func(ctx context.Context) {
			...
			suite.K2sCli().Exec(ctx, "addons", "disable", "metrics", "-o", "-f")
	...
	Describe("export and import with relative paths", func() {
		...
		BeforeAll(func(ctx context.Context) {
			absRelExportDir = filepath.Join(suite.RootDir(), "tmp", "metrics-relpath-test")
		...
		It("exports addon using a relative directory path", func(ctx context.Context) {
			relOciFile = exportimport.ExportAddonRelativePath(ctx, suite, "metrics", "metrics", suite.RootDir(), relExportDir)
