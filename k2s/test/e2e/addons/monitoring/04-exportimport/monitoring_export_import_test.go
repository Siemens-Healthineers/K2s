...
func TestMonitoringExportImport(t *testing.T) {
	...
	RunSpecs(t, "monitoring Addon Export/Import Tests", Label("addon", "addon-ilities", "acceptance", "internet-required", "setup-required", "invasive", "monitoring", "export-import", "air-gapped", "system-running"))
}
...
var _ = Describe("monitoring addon export and import", Ordered, func() {
	...
	Describe("export and import with relative paths", func() {
		...
		BeforeAll(func(ctx context.Context) {
			absRelExportDir = filepath.Join(suite.RootDir(), "tmp", "monitoring-relpath-test")
		...
		It("exports addon using a relative directory path", func(ctx context.Context) {
			relOciFile = exportimport.ExportAddonRelativePath(ctx, suite, "monitoring", "monitoring", suite.RootDir(), relExportDir)
