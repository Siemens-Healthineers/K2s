...
func NewAddUserIntegration(k2sConfig *config.K2sConfig, runtimeConfig *config.K2sRuntimeConfig, usersProvider UsersProvider) *AddUserIntegration {
	...
	kubeconfigReader := kubeconfig_adapter.NewKubeconfigReader(k2sConfig.Host().KubeConfig())
	...
	kubeconfigCopier := kubeconfig.NewKubeconfigCopier(kubeconfigReader, clusterFinder, kubeconfigWriter)
	...
	kubeconfigApiCredsReader := kubeconfig.NewKubeconfigReader(k2sConfig.Host().KubeConfig(), kubeconfigReader, credentialsFinder)
	accessVerifier := cluster.NewClusterAccessVerifier(kubeconfigApiCredsReader, credentialsDecoder, apiAccessVerifier)
	clusterAdmission := cluster.NewClusterAdmission(runtimeConfig.ClusterConfig(), kubeconfigResolver, kubeconfigCopier, kubeconfigWriter, certGenerator, accessVerifier)
