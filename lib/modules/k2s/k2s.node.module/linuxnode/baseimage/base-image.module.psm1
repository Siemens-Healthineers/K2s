...
function Invoke-Download($destination, $source, $forceDownload,
    ...
    if ( $ProxyToUse -ne '' ) {
        ...
        curl.exe --retry 5 --connect-timeout 60 --retry-all-errors --retry-delay 60 --silent --disable --fail -Lo $destination $source --proxy $ProxyToUse #ignore server certificate error for cloudbase.it
    }
    else {
...
function Invoke-DownloadDebianImage {
    ...
    if ([System.IO.File]::Exists($imgFile)) {
        ...
        if ( $Proxy -ne '') {
            ...
            $allHashes = curl.exe --retry 3 --connect-timeout 60 --retry-connrefused --silent --disable --fail "$urlRoot/SHA512SUMS" --proxy $Proxy -k
        }
        else {
