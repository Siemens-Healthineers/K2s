...
function Add-LocalIPAddress {
    ...
    [string]$gateway = Invoke-CmdOnControlPlaneViaUserAndPwd -CmdToExecute "ip route | awk '/default/ {print `$3}'" -RemoteUser "$remoteUser" -RemoteUserPwd "$remoteUserPwd" -NoLog
...
function Add-RemoteIPAddress {
    ...
    $executeRemoteCommand = {
        ...
        if ($NoLog) {
            Invoke-CmdOnControlPlaneViaUserAndPwd -CmdToExecute $command -RemoteUser "$remoteUser" -RemoteUserPwd "$remoteUserPwd" -NoLog
        } else {
            Invoke-CmdOnControlPlaneViaUserAndPwd -CmdToExecute $command -RemoteUser "$remoteUser" -RemoteUserPwd "$remoteUserPwd"
        }
    }
