...
function Remove-DuplicateImages {
    ...
    if ($null -eq $deduplicatedImages -or $deduplicatedImages.Count -eq 0) {
        Write-Log "[$NodeType`Node] No images remaining after deduplication, returning empty array"
        return @()
    }

    Write-Log "[$NodeType`Node] Deduplication complete: $($ContainerImages.Count) input image(s) -> $($deduplicatedImages.Count) output image(s)"
