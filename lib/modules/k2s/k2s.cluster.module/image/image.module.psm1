...
function New-WindowsImage {
    ...
    &$ctrExe -n k8s.io images import "$exportedImageFullFileName"
    if (!$?) { throw "error while importing built image '$imageFullName' with 'ctr images import' on Windows. Error code returned was $LastExitCode" }
    Write-Log '...imported'
