...
k2s_load_operation() {
  ...
  K2S_VERSION=$(jq -er '.version // empty' "$2")
  K2S_CLUSTER_NAME=$(jq -er '.clusterName // "k2s-cluster"' "$2")
  K2S_CONTROL_PLANE_HOSTNAME=$(jq -er '.controlPlaneHostname // empty' "$2")
  K2S_PROXY=$(jq -er '.proxy // empty' "$2")
  K2S_NO_PROXY=$(jq -er '.noProxy // [] | join(",")' "$2")
  K2S_SKIP_START=$(jq -er '.skipStart // false' "$2")
  K2S_SKIP_PURGE=$(jq -er '.skipPurge // false' "$2")
...
k2s_lock_and_run() { mkdir -p "$K2S_CONFIG_DIR"; exec 9>"$K2S_CONFIG_DIR/lifecycle.lock"; flock -w 300 9 || return 1; "$@"; }
k2s_cfg() { jq -er "$1" "$K2S_INSTALL_DIR/cfg/config.json"; }
k2s_kubectl() { kubectl --kubeconfig /etc/kubernetes/admin.conf "$@"; }

k2s_require_host() {
  k2s_require_root || return 3; k2s_require_install_dir || return 1
  [[ $(. /etc/os-release; printf '%s-%s' "$ID" "$VERSION_ID") == debian-13* ]] || return 3
  local tool; for tool in jq flock systemctl apt-get dpkg modprobe sysctl chattr lsattr ip; do k2s_require_command "$tool" || return 4; done
...
EOF
  systemctl daemon-reload; systemctl enable --now k2s-proxy-network
}
...
EOF
  ...
  systemctl daemon-reload; systemctl enable --now k2s-httpproxy
}

k2s_packages_install() {
  local version token gateway; version="$K2S_VERSION"; [[ -n "$version" ]] || version=$(grep -Eo "v[0-9]+\.[0-9]+\.[0-9]+" "$K2S_INSTALL_DIR/lib/modules/windows/infra/k2s.infra.module/config/config.module.psm1" | head -1)
  ...
  "$K2S_INSTALL_DIR/lib/scripts/linux/debian/linuxonly/ProvisionPackages.sh" download "$K2S_CONFIG_DIR/packages" "$version" "http://$gateway:8181"
  "$K2S_INSTALL_DIR/lib/scripts/linux/debian/linuxonly/ProvisionPackages.sh" install "$K2S_CONFIG_DIR/packages" "http://$gateway:8181" "$token" false "$K2S_NO_PROXY" true
  command -v kubeadm kubectl crictl crio >/dev/null
}
...
EOF
  systemctl enable --now crio; kubeadm init --config="$cfg"; rm -f "$cfg"; systemctl enable kubelet || true
...
WantedBy=multi-user.target
EOF
  systemctl daemon-reload; systemctl enable --now k2s-dnsproxy
...
k2s_cluster_install() {
  ...
  k2s_require_host; k2s_proxy_network_install; k2s_proxy_install; k2s_packages_install; k2s_control_plane_install
  ...
  sed -e 's/NETWORK.NAME/cbr0/g' -e "s/NETWORK.ADDRESS/$(k2s_cfg '.smallsetup.podNetworkCIDR')/g" -e 's/NETWORK.TYPE/vxlan/g' "$flannel" | k2s_kubectl apply -f -
  for manifest in namespace.yaml rbac.yaml webhook-config.yaml deployment.yaml; do k2s_kubectl apply -f "$K2S_INSTALL_DIR/lib/manifests/clusterip-webhook/$manifest"; done
...
k2s_start_cluster() { systemctl start k2s-proxy-network k2s-httpproxy crio kubelet; local end=$((SECONDS+60)); until k2s_kubectl cluster-info >/dev/null 2>&1; do ((SECONDS<end)) || return 1; sleep 3; done; k2s_dns_start; }
...
k2s_dispatch_lifecycle() { local op="$1"; shift; k2s_load_operation "$@" || return $?; k2s_initialize_logging; k2s_lock_and_run "k2s_${op}_cluster"; }
