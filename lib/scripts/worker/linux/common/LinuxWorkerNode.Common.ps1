...
If specified, waits for the node to transition to Ready state (up to 60 seconds).

.PARAMETER IncludeHyperVDiagnostics
If specified, includes Hyper-V specific diagnostics on SSH failure (for VM-based nodes).
...
function Start-LinuxWorkerNodeServices {
    Param(
        ...
        [switch] $IncludeHyperVDiagnostics = $false,
...
function Invoke-LinuxWorkerNodeStart {
    ...
    if ($null -ne $nodeConfig -and -not [string]::IsNullOrWhiteSpace($nodeUserName) -and -not [string]::IsNullOrWhiteSpace($nodeConfig.IpAddress)) {
        Start-LinuxWorkerNodeServices -NodeName $workerNodeName -IpAddress $nodeConfig.IpAddress -UserName $nodeUserName -WaitForReady:$WaitForReady -IncludeHyperVDiagnostics:$IncludeHyperVDiagnostics -LogPrefix $LogPrefix
    } else {
