...
k2s_require_root || { k2s_log ERROR 'Native Debian 13 installation requires root privileges.'; exit 1; }
...
k2s_log INFO 'Dispatching native Debian 13 Linux-only installation through the K2s CLI.'
exec "$k2s_cli" install --linux-only "$@"