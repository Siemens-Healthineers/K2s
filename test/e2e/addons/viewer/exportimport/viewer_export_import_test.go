...
func TestViewerExportImport(t *testing.T) {
	RegisterFailHandler(Fail)
	RunSpecs(t, "viewer Addon Export/Import Tests", Label("addon", "addon-medical", "acceptance", "internet-required", "setup-required", "invasive", "viewer", "export-import", "air-gapped", "system-running"))
}

var _ = BeforeSuite(func(ctx context.Context) {
	GinkgoWriter.Println("========================================")
	GinkgoWriter.Println("VIEWER EXPORT/IMPORT TEST - SETUP")
	GinkgoWriter.Println("========================================")
	...
	exportPath = filepath.Join(suite.RootDir(), "tmp", "viewer-export-test")
	...
	addon = exportimport.GetAddonByName(allAddons, "viewer")
	Expect(addon).NotTo(BeNil(), "viewer addon should exist")
	GinkgoWriter.Printf("[Setup] Found addon: %s\n", addon.Metadata.Name)

	impl = exportimport.GetImplementation(addon, "viewer")
	Expect(impl).NotTo(BeNil(), "viewer implementation should exist")
...
var _ = Describe("viewer addon export and import", Ordered, func() {
	Describe("export viewer addon", func() {
		BeforeAll(func(ctx context.Context) {
			exportimport.CleanupExportedFiles(exportPath, "")
		})

		It("exports viewer addon to versioned OCI tar file", func(ctx context.Context) {
			GinkgoWriter.Println(">>> TEST: exports viewer addon to versioned OCI tar file")
			exportedOciFile = exportimport.ExportAddon(ctx, suite, "viewer", "", exportPath)
		...
		It("contains viewer addon folder with correct OCI structure", func(ctx context.Context) {
			GinkgoWriter.Println(">>> TEST: contains viewer addon folder with correct OCI structure")
			extractedArtifactsDir := exportimport.ExtractOciTar(ctx, suite, exportedOciFile, exportPath)

			expectedDirName := exportimport.GetExpectedDirName("viewer", "viewer")
	...
	Describe("clean up viewer resources", func() {
		BeforeAll(func(ctx context.Context) {
	...
	Describe("import viewer addon", func() {
		...
		It("can be enabled when only addons/common and addons/viewer are present", func(ctx context.Context) {
			...
			DeferCleanup(func() {
				_, _ = suite.K2sCli().Exec(context.Background(), "addons", "disable", "viewer", "-o")
			})
	...
	Describe("export and import with relative paths", func() {
		...
		BeforeAll(func(ctx context.Context) {
			absRelExportDir = filepath.Join(suite.RootDir(), "tmp", "viewer-relpath-test")
		...
		It("exports addon using a relative directory path", func(ctx context.Context) {
			relOciFile = exportimport.ExportAddonRelativePath(ctx, suite, "viewer", "viewer", suite.RootDir(), relExportDir)
