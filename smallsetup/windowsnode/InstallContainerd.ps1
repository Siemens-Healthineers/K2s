# SPDX-FileCopyrightText: © 2023 Siemens Healthcare GmbH
#
# SPDX-License-Identifier: MIT

#Requires -RunAsAdministrator
Param(
    [parameter(Mandatory = $false, HelpMessage = 'HTTP proxy if available')]
    [string] $Proxy = ''
)
# load global settings
&$PSScriptRoot\..\common\GlobalVariables.ps1
. $PSScriptRoot\..\common\GlobalFunctions.ps1

function Set-RootPathForImagesInConfig($tomlPath) {
    $template = $tomlPath + '.template'
    if (Test-Path $template) {
        $storageLocalDrive = Get-StorageLocalDrive
        (Get-Content -path $template -Raw) -replace '%BEST-DRIVE%', $storageLocalDrive | Set-Content -Path $tomlPath
    }
}

function Set-InstallationDirectory($tomlPath) {
    if (!(Test-Path $tomlPath)){
        throw "File '$tomlPath' not found"
    }
    $formattedInstallationDirectory = $global:KubernetesPath.Replace("\", "\\");
    (Get-Content -path $tomlPath -Raw) -replace '%INSTALLATION_DIRECTORY%', $formattedInstallationDirectory | Set-Content -Path $tomlPath
}

function Set-UserTokenForRegistryInConfig($tomlPath) {
    # containerd config toml
    $token = Get-RegistryToken
    (Get-Content -path $tomlPath -Raw) -replace '%CONTAINERD_TOKEN%', $token | Set-Content -Path $tomlPath

    # nerdctl (default refers to .docker/config.json)
    $jsonConfig = @{
        auths = @{
            "shsk2s.azurecr.io" = @{
                auth = $token
            }
        }
    }

    $jsonString = ConvertTo-Json -InputObject $jsonConfig
    New-Item -Path "$env:userprofile\.docker\config.json" -ItemType File -Force | Out-Null
    $jsonString | Set-Content -Path "$env:userprofile\.docker\config.json"
}

Write-Log "First uninstall containerd service if existent"
&"$global:KubernetesPath\smallsetup\windowsnode\UninstallContainerd.ps1"

Write-Log "Start publishing containerd artifacts"
&"$global:KubernetesPath\smallsetup\windowsnode\publisher\PublishContainerd.ps1"
Write-Log "Finished publishing containerd artifacts"

Write-Log "Creating crictl.yaml (config for crictl.exe)"
@'
runtime-endpoint: npipe://./pipe/containerd-containerd
image-endpoint: npipe://./pipe/containerd-containerd
timeout: 30
#debug: true
'@ | Set-Content "$global:KubernetesPath\bin\crictl.yaml" -Force

# install the network plugins
# $plfile = 'windows-container-networking-cni-amd64-v0.3.0.zip'
# if (!(Test-Path "$global:KubernetesPath\containerd\$plfile")) {
#     DownloadFile $global:KubernetesPath\containerd\$plfile https://github.com/microsoft/windows-container-networking/releases/download/v0.3.0/$plfile
# }
# powershell Expand-Archive $global:KubernetesPath\containerd\$plfile -DestinationPath $global:KubernetesPath\cni\bin -Force
# if (!$?) { throw "unable to extract $plfile" }
#Remove-Item -Path $global:KubernetesPath\containerd\$plfile -Force -Recurse -ErrorAction SilentlyContinue

if ($(Get-Installedk2sSetupType) -ne $global:SetupType_BuildOnlyEnv) {
    # replace local ip in cni template config
    Write-Log "Create CNI config file"
    mkdir "$global:KubernetesPath\cfg\containerd\cni\conf" -ErrorAction SilentlyContinue | Out-Null
    Copy-Item "$global:KubernetesPath\cfg\containerd\flannel-l2bridge.conf.template" "$global:KubernetesPath\cfg\containerd\flannel-l2bridge.conf" -Force

    $adapterName = Get-L2BridgeNIC
    Write-Log "Using network adapter '$adapterName'"
    $ipaddresses = @(Get-NetIPAddress -AddressFamily IPv4 -InterfaceAlias $adapterName)
    if (!$ipaddresses) {
        throw 'No IP address found on the host machine which can be used for setting up networking  !'
    }
    $ipaddress = $ipaddresses[0] | Select-Object -ExpandProperty IPAddress
    Write-Log "Using local IP $ipaddress for setup of CNI"

    $nameServers = Get-Content "$global:KubernetesPath\cfg\containerd\flannel-l2bridge.conf" | Select-String "NAME.SERVERS" | Select-Object -ExpandProperty Line
    if ( $nameServers ) {
        $configuredNameservers = ""
        $global:ClusterCIDR_NextHop | ForEach-Object { $configuredNameservers += "                                ""$_"",`n" }
        $global:KubeDnsServiceIP | ForEach-Object { $configuredNameservers += "                                ""$_""" }

        $content = Get-Content "$global:KubernetesPath\cfg\containerd\flannel-l2bridge.conf"
        $content | ForEach-Object { $_ -replace $nameServers, $configuredNameservers } | Set-Content "$global:KubernetesPath\cfg\containerd\flannel-l2bridge.conf"
    }

    $natExceptions = Get-Content "$global:KubernetesPath\cfg\containerd\flannel-l2bridge.conf" | Select-String "NAT.EXCEPTIONS" | Select-Object -ExpandProperty Line
    if ( $natExceptions ) {

        $configuredExceptions = ""
        $global:ClusterCIDR_NatExceptions | ForEach-Object { $configuredExceptions += "                            ""$_"",`n" }
        $content = Get-Content "$global:KubernetesPath\cfg\containerd\flannel-l2bridge.conf"
        $network2 = $ipaddress.Remove($ipaddress.LastIndexOf('.')) + '.0'
        $network2 = "                            ""$network2/24"""

        if ($configuredExceptions -ne "") {
            $configuredExceptions += $network2
        } else {
            $configuredExceptions = $network2
        }

        $content | ForEach-Object { $_ -replace $natExceptions, $configuredExceptions } | Set-Content "$global:KubernetesPath\cfg\containerd\flannel-l2bridge.conf"
    }
    Move-Item -Path "$global:KubernetesPath\cfg\containerd\flannel-l2bridge.conf" -Destination "$global:KubernetesPath\cfg\containerd\cni\conf" -ErrorAction SilentlyContinue
}
# register and start containerd
Write-Log "Register service and start containerd service"
Set-RootPathForImagesInConfig "$global:KubernetesPath\cfg\containerd\config.toml" | Out-Null
Set-InstallationDirectory "$global:KubernetesPath\cfg\containerd\config.toml" | Out-Null
Set-UserTokenForRegistryInConfig "$global:KubernetesPath\cfg\containerd\config.toml" | Out-Null
$target = "$($global:SystemDriveLetter):\var\log\containerd"
Remove-Item -Path $target -Force -Recurse -ErrorAction SilentlyContinue | Out-Null
mkdir "$($global:SystemDriveLetter):\var\log\containerd" -ErrorAction SilentlyContinue | Out-Null
&$global:NssmInstallDirectory\nssm install containerd $global:KubernetesPath\bin\containerd\containerd.exe *>&1 | ForEach-Object { $_.Trim() }
&$global:NssmInstallDirectory\nssm set containerd AppDirectory $global:KubernetesPath\bin\containerd | Out-Null
&$global:NssmInstallDirectory\nssm set containerd AppParameters "--log-file=\`"$($global:SystemDriveLetter):\var\log\containerd\logs.log\`" --config \`"$global:KubernetesPath\cfg\containerd\config.toml\`"" | Out-Null
&$global:NssmInstallDirectory\nssm set containerd AppStdout "$($global:SystemDriveLetter):\var\log\containerd\containerd_stdout.log" | Out-Null
&$global:NssmInstallDirectory\nssm set containerd AppStderr "$($global:SystemDriveLetter):\var\log\containerd\containerd_stderr.log" | Out-Null
&$global:NssmInstallDirectory\nssm set containerd AppStdoutCreationDisposition 4 | Out-Null
&$global:NssmInstallDirectory\nssm set containerd AppStderrCreationDisposition 4 | Out-Null
&$global:NssmInstallDirectory\nssm set containerd AppRotateFiles 1 | Out-Null
&$global:NssmInstallDirectory\nssm set containerd AppRotateOnline 1 | Out-Null
&$global:NssmInstallDirectory\nssm set containerd AppRotateSeconds 0 | Out-Null
&$global:NssmInstallDirectory\nssm set containerd AppRotateBytes 500000 | Out-Null
&$global:NssmInstallDirectory\nssm set containerd Start SERVICE_AUTO_START | Out-Null

Write-Log "Proxy to use with containerd: '$Proxy'"
if ( $Proxy -ne '' ) {
    &$global:NssmInstallDirectory\nssm set containerd AppEnvironmentExtra HTTP_PROXY=$Proxy HTTPS_PROXY=$Proxy NO_PROXY=.local | Out-Null
}

# add firewall entries (else firewall will keep your CPU busy)
Write-Log "Adding firewall rules for containerd"
New-NetFirewallRule -DisplayName "Containerd" -Group "k2s" -Direction Inbound -Action Allow -Program "$global:KubernetesPath\bin\containerd\containerd.exe" -Enabled True | Out-Null
New-NetFirewallRule -DisplayName "Containerd-Shim" -Group "k2s" -Direction Inbound -Action Allow -Program "$global:KubernetesPath\bin\containerd\containerd-shim-runhcs-v1.exe" -Enabled True | Out-Null

# start containerd service
Start-Service containerd -WarningAction SilentlyContinue

# ensure service is running
$expectedServiceStatus = "SERVICE_RUNNING"
Write-Log "Waiting until service 'containerd' has status '$expectedServiceStatus'"
$retryNumber = 0
$maxAmountOfRetries = 3
$waitTimeInSeconds = 2
$serviceIsRunning = $false
while ($retryNumber -lt $maxAmountOfRetries) {
    $serviceStatus = (&$global:NssmInstallDirectory\nssm status containerd)
    if ($serviceStatus -eq "$expectedServiceStatus") {
        $serviceIsRunning = $true
        break;
    }
    $retryNumber++
    Start-Sleep -Seconds $waitTimeInSeconds
    $totalWaitingTime = $waitTimeInSeconds * $retryNumber
    Write-Log "Waiting since $totalWaitingTime seconds for service 'containerd' to be in status '$expectedServiceStatus' (current status: $serviceStatus)"
}
if (!$serviceIsRunning) {
    throw "Service 'containerd' is not running."
}
Write-Log "Service 'containerd' has status '$expectedServiceStatus'"

