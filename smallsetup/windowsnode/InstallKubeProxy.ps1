...
$hn = ($(hostname)).ToLower()
&$global:NssmInstallDirectory\nssm set kubeproxy AppParameters "--proxy-mode=kernelspace --hostname-override=$hn --kubeconfig=\`"$global:KubernetesPath\config\`" --enable-dsr=false --log-dir=\`"$($global:SystemDriveLetter):\var\log\kubeproxy\`" --logtostderr=false" | Out-Null
