...
Function Get-DebianImage {
    ...
    $params = @{Proxy=$Proxy
                OutputPath=$DownloadsDirectory}
    $imgFile = Invoke-ScriptFile -ScriptPath "$PSScriptRoot\..\common\vmtools\Get-DebianImage.ps1" -Params $params
...
Function New-RootfsForWSL {
    ...
    $executeRemoteCommand = {
        ...
        if ($IgnoreErrors) {
            ...
            ExecCmdMaster $command -RemoteUser "$user" -RemoteUserPwd "$userPwd" -UsePwd
        }
    }
...
Function Get-QemuExecutable {
    ...
    if (!(Test-Path $zipFilePath)) {
        ...
        DownloadFile $zipFilePath $url $true $Proxy
        Write-Log "  ...done"
    }
    else {
...
function Copy-LocalPublicSshKeyToRemoteComputer() {
    ...
    $remoteTargetPath = "$user" + ":$targetPath"
    Copy-FromToMaster -Source $localSourcePath -Target $remoteTargetPath -RemoteUserPwd "$userPwd" -UsePwd
    ExecCmdMaster "sudo mkdir -p ~/.ssh" -RemoteUser "$user" -RemoteUserPwd "$userPwd" -UsePwd
    ExecCmdMaster "sudo cat $targetPath | sudo tee ~/.ssh/authorized_keys" -RemoteUser "$user" -RemoteUserPwd "$userPwd" -UsePwd
}
